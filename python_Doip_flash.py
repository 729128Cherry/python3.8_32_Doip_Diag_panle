...
class Diag():
    ...
    def socket_disconect(self, socket_obj):
        try:
            ...
            demo.socket_connect_button.configure(bg="yellow", text="直连-alm")
    ...
    @With_Thread
    def socket_disconect_cgw(self, socket_obj):
        try:
            socket_obj.shutdown(2)
            socket_obj.close()
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), ":socket已关闭！")
            demo.socket_connect_button_cgw.configure(bg="blue", text="- -", command=lambda: demo.UDS.socket_conect_cgw())
        except:
            tkinter.messagebox.showwarning('警告', 'socket关闭失败！')
            demo.socket_connect_button_cgw.configure(bg="yellow", text="alm")

    @With_Thread
    def socket_conect_cgw(self):
        try:
            global global_socket
            global_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            global_socket.connect(("169.254.3.17", 13400))
            demo.socket_connect_button_cgw.configure(bg="green", text="cgw-on",
                                                 command=lambda: demo.UDS.socket_disconect_cgw(global_socket))
            self.socket_rx(global_socket)
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), " :socket已连接！")

            # 路由激活
            router_active_req_data = "02fd00050000000b0e80000000000000000000"
            global_socket.send(bytes.fromhex(router_active_req_data))
            router_active_rsp_data = global_socket.recv(1024).hex()
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" : 路由激活响应 ：{router_active_rsp_data}")
        except Exception as e:
            print(e)
            tkinter.messagebox.showwarning('警告', 'socket连接失败！')
            demo.socket_connect_button_cgw.configure(bg="yellow", text="cgw-alm")
    ...
    def resp_deal_flash(self, socket_obj, rsp_head, rsp_tail, req, rsp_40p, rsp_78, pro=None, seqnum=None, size=None):
        while True:
            ...
            if ret == 120:
                continue
            elif ret[rsp_head:rsp_tail] == rsp_40p:
    ...
    def flash_36(self, flash_socket, f, size=None, flash_size=None):
        file_szie = size
        while True:
            for seq in range(0, 256):
                x = f.read(flash_size)
                file_szie -= flash_size
                if file_szie < -flash_size:
    ...
    def flash_F15C(self, socket_obj):
        while True:
            ...
            if ret[-8:-2] == "62f15c":
                ...
                continue
            elif ret == "02fd800200000005e4000e8000":
                continue
            elif ret == "OSError: [WinError 10038]" or 'Null':
    ...
    @With_Thread
    def flash_con(self, fin, flash_socket):
        # 会话保持
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" : 会话保持")
        while flash_flag:
            router_active_req_data = "02fd8001000000060e80"+fin+"3E80"
            flash_socket.send(bytes.fromhex(router_active_req_data))
            time.sleep(3)
    ...
    def doip_falsh(self, file_path):
        ...
        # 36
            ...
            ret = self.flash_36(flash_socket, f, size=file_size, flash_size=65536)
            if ret == 0:
                return None
    ...
    @With_Thread
    def doip_falsh_cgw(self, file_path):
        global flash_flag
        tkinter.messagebox.showwarning('警告', '刷写期间禁止使用“诊断命令面板按钮、选项”！')
        flash_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        flash_socket.connect(("169.254.3.17", 13400))
        flash_socket.settimeout(2)
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), " : socket已连接！")
        ...
        router_active_req_data = "02fd00050000000b0e80000000000000000000"
        flash_socket.send(bytes.fromhex(router_active_req_data))
        router_active_rsp_data = flash_socket.recv(1024).hex()
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" : 路由激活响应 ：{router_active_rsp_data}")
        ...
        # # 会话保持
        # self.flash_con("16E2", flash_socket)
        ...
        with open(str(flash_file_path), "rb") as f:
            ...
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                  f" 36 step : 文件大小为{os.path.getsize(str(flash_file_path))}！")

            # 3803
            self.socket_tx_flash(flash_socket, "16e2",
                                 "3803001E443A5C6D6170646174615C6575726F70655C6765726D616E79312E79787A1104" + (
                                     hex(file_size).replace("0x", "").zfill(8)) * 2)
            ret = self.resp_deal_flash(flash_socket, -16, -12, "3803", "7803", "7f3878", 12)
            if ret == 0:
                return None
            ...
            for seq in range(1, 256):
                ...
                while 1:
                    self.socket_tx_flash(flash_socket, "16e2", "36" + hex(seq).replace("0x", "").zfill(2), x)
                    ret = self.resp_deal_flash_36(flash_socket, -4, -2, "36", "76", "7f3678", seqnum=seq,
                                                  size=file_size)
                    if ret == 0:
                        return None
                    elif ret == 255:
                        ...
                        break
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" 36 step RX: 块1传输完毕！")
            ret = self.flash_36(flash_socket, f, size=file_size, flash_size=4096)
            if ret == 0:
                return None
...
class GUI(tk.Tk):
    ...
    def _Third_datainput_WidgetsInit(self):
        self.socket_connect_button_cgw = tk.Button(self._Second_datainput, bg="blue", width=2, text='route 网关',command=lambda: self.UDS.socket_conect_cgw())
        self.socket_connect_button_cgw.grid(row=0, column=0, padx=5, pady=2.5, sticky=tk.E + tk.W)

        self.socket_connect_button=tk.Button(self._Second_datainput,bg="blue",width=5,text='直连',command=lambda: self.UDS.socket_conect())
        self.socket_connect_button.grid(row=0, column=1, padx=4, pady=2.5, sticky=tk.E + tk.W)
    ...
    def _Third_dataflash_WidgetsInit(self):
        ...
        tk.Button(self._Second_dataflash, text='直连刷写', command=lambda: self.UDS.doip_falsh(self.path)).grid(row=1, column=2, padx=5, pady=5)
        tk.Button(self._Second_dataflash, text='CGW刷写', command=lambda: self.UDS.doip_falsh_cgw(self.path)).grid(row=1, column=3, padx=5, pady=5)
        pass
